...
const KorluhSayuranMobile = ({ urlApi }: KorluhRespon) => {
    // Format date
    ...
    // GETALL
    const { data: dataSayuran } = useSWR<KorluhSayurBuahResponse>(
        // `/korluh/sayur-buah/get?page=${currentPage}&search=${search}&startDate=${filterDate}`,
        urlApi,
        (url: string) =>
            axiosPrivate
                .get(url, {
    ...
    const columnsTanaman = [
        ...
        { id: "hasilProduksi", text: "Hasil Produksi (kuintal)" },
    ...
    const tableDataTanaman: Array<Record<string, React.ReactNode>> = dataSayuran?.data?.data.flatMap((item) =>
        item.list.map((tanaman) => ({
            Tanaman: tanaman.namaTanaman,
            hasilProduksi: tanaman.hasilProduksi,
    ...
    return (
        <div className="">
            ...
            <div className="wrap-table flex flex-col gap-1">
                <div className="font-semibold mb-2">Jenis Sayuran dan Buah</div>
...
export default KorluhSayuranMobile;
