    ...
    return (
        ...
        <>
            <CardContainer className="inter-var">
                <Link href={`/berita/${props.slug}`} className="card rounded-xl bg-primary-600 p-4 pb-8">
                    <CardItem translateZ="50" className="image rounded overflow-hidden h-[200px] w-full">
                        <Image src={props.image || "../../assets/images/cardberita.png"} alt="logo" width={300} height={300} unoptimized className='w-full h-full object-cover group-hover/card:shadow-xl' />
                    </CardItem>
                    <CardItem className="wrap-teks flex flex-col gap-3">
                        <CardItem translateZ="50" className="date mt-4 text-[#2C2C2C] text-[12px]">
                            {props.date && !isNaN(new Date(props.date).getTime()) ? (
                                new Date(props.date).toLocaleDateString('id-ID', {
                                    day: 'numeric',
                                    month: 'long',
                        ...
                        <CardItem translateZ="50" className="wrap-title flex gap-1 items-start">
                            <div className="title text-[#2C2C2C] text-[16px] font-semibold line-clamp-2">
                                {props.title || "Title"}
                            </div>
                            <div className="logo pt-1">
