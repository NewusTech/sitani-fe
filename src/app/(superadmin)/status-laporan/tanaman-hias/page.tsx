...
const StatusLaporanTanamanHias = () => {
    ...
    const { data: dataStatus }: SWRResponse<any> = useSWR(
        `/status-laporan/hias?kecamatan=${selectedKecamatan}&tahun=${tahun}&status=${status === "semua" ? "" : status}`,
        (url) =>
            axiosPrivate
                .get(url, {
                    headers: {
    ...
    const handleAjukanKembaliFunction = async (id: number) => {
        try {
            const response = await axiosPrivate.post(`/validasi/korluh-tanaman-hias/update/${id}`, {
                status: "tunggu"
            });
            // alert
            Swal.fire({
                        ...
                        <div key={index} className="card-table text-[12px] p-4 rounded-2xl border border-primary bg-white shadow-sm">
                            <div className="wrap-konten flex flex-col gap-2">
                                <div className="flex justify-between gap-5">
                                    <div className="label font-medium text-black">Triwulan</div>
...
export default StatusLaporanTanamanHias