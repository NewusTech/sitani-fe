...
interface PenyuluhKelompokTani {
    status: number;
    message: string;
    data: {
        id: number;
...
const formSchema = z.object({
    id_poktan: z
        .string()
        .min(0, { message: "Id Poktan wajib diisi" }),
    kecamatan_id: z
        .number()
...
const PoktanEditDataKabupaten = () => {
    ...
    const { data: dataUser, error } = useSWR<PenyuluhKelompokTani>(
        `penyuluh-kelompok-tani/get/${id}`,
        async (url: string) => {
            try {
    ...
    useEffect(() => {
        if (dataUser) {
            const timeoutId = setTimeout(() => {
                ...
                setValue("id_poktan", dataUser.data.idPoktan.toString());
                ...
                setValue("dibent", dataUser.data.dibent.toString());
                setValue("l", dataUser.data.l.toString());
                setValue("p", dataUser.data.p.toString());
                setValue("kelas", dataUser.data.kelas);
    ...
    const onSubmit: SubmitHandler<FormSchemaType> = async (data) => {
        setLoading(true);
        try {
            await axiosPrivate.put(`/penyuluh-kelompok-tani/update/${id}`, data);
            Swal.fire({
                icon: 'success',
            ...
            navigate.push('/penyuluhan/data-poktan');
            reset();
        } catch (error: any) {
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="">
                <div className="wrap-form text-sm">
                    <div className="mb-2">
                        ...
                        <div className="flex md:flex-row flex-col justify-between gap-2 md:lg-3 lg:gap-5">
                            <div className="flex flex-col mb-2 w-full">
                                <Label className='text-sm mb-1' label="ID Poktan" />
                                <Input
                                    type="number"
                                    placeholder="ID Poktan"
                                    {...register('id_poktan')}
                                    className={`${errors.id_poktan ? 'border-red-500' : 'py-5 text-sm'}`}
                                />
                                {errors.id_poktan && (
                                    <HelperError>{errors.id_poktan.message}</HelperError>
                                )}
                            </div>
                            <div className="flex flex-col mb-2 w-full">
                        ...
                        <div className="flex md:flex-row flex-col justify-between gap-2 md:lg-3 lg:gap-5">
                            ...
                            <div className="flex flex-col mb-2 w-full">
                                ...
                                <Input
                                    ...
                                    {...register('dibent')}
                                    className={`${errors.dibent ? 'border-red-500' : 'py-5 text-sm'}`}
                                />
                                {errors.dibent && (
                                    <HelperError>{errors.dibent.message}</HelperError>
                                )}
                            </div>
                        </div>
                        {/* jumlah anggota */}
                        ...
                        <div className="flex md:flex-row flex-col justify-between gap-2 md:lg-3 lg:gap-5">
                            <div className="flex flex-col mb-2 md:w-1/2 md:pr-3 w-full">
                                <Label className='text-sm mb-1' label="Kelas Kelompok" />
                                <Select
                                    onValueChange={(value) => setValue("kelas", String(value))}
                                    value={String(watch('kelas')) || ''}>
                                    <SelectTrigger className="w-full">
                                        <SelectValue placeholder="Kelas Kelompok" />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="p">Pemula</SelectItem>
                                        <SelectItem value="l">Lanjut</SelectItem>
                                        <SelectItem value="m">Madya</SelectItem>
                                        <SelectItem value="u">Utama</SelectItem>
                                    </SelectContent>
                                </Select>
                                {errors.kelas && (
                                    <HelperError>{errors.kelas.message}</HelperError>
                                )}
                            </div>
                        </div>
                    </div>
...
export default PoktanEditDataKabupaten;
